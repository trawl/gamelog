...
def decodePhase(phasecode):
    types = {
        "c": "cartas del mismo color",
        "r": ["escalera de", "escaleras de"],
        "cr": ["escalera de color de", "escaleras de color de"],
    }
    sets = {
        "2": ["pareja", "parejas"],
        "3": ["trío", "tríos"],
        "4": ["cuarteto", "cuartetos"],
        "5": ["quinteto", "quintetos"],
    }
    ...
    for part in phasecode.split():
        ...
        if m:
            ...
            first = False
            if tcode == "s":
                desc += f"{n} {sets[cards][plural]}"
            elif tcode == "c":
                desc += f"{cards} {types[tcode]}"
            elif tcode in ["r", "cr"]:
                desc += f"{n} {types[tcode][plural]} {cards}"
    return desc
